...
<?php 
    ...
    $student_info = DB::query($student_info, array(':usr_id'=>$student_id))[0];


   
    $messages = "SELECT * FROM tbl_message WHERE 
    crs_id='". $crs_id ."' AND 
    from_usr_id='". $student_info['usr_id'] ."' || 
    (from_usr_id='". $user_info['usr_id'] ."' AND to_usr_id='". $student_info['usr_id'] ."') 
...
<div class="container">
    ...
    <a href="message.php?course=<?= $_GET['course'] ?>" class="btn btn-info btn-sm"><b><</b> Back</a> 
...
<div class="row">
    <div class="col-sm-12">
        <img src="../../<?= $student_info['profilepic'] ?>" alt="Prifile Pic" class="cmmnt-pp">
        <label><?= $student_info['firstname'] . " ". $student_info['middlename'] ." ". $student_info['lastname'] ?></label>
            ...
            <?php foreach($messages as $message){ ?>
                <p class="comment-form" style="margin-top: 10px; <?= ($message['from_usr_id'] == $user_info['usr_id']) ? 'border-color: rgb(51, 122, 183)!important;': ''?>">
                        ...
                        <?php     
                         ...
                         ?>
                            <img src="../../<?= $student_info['profilepic'] ?>" style="height: 25px; width: 25px" class="pp">
                            <b> <?= $student_info['firstname'] ." ". $student_info['lastname'] ?> : </b>
                         <?php  
                        }
                         ?> 
                ...
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">
                        ...
                        <span class="text-primary">
                            <?= $student_info['firstname'] . " ". $student_info['middlename'] ." ". $student_info['lastname'] ?>
                        </span>
                    </h4>
                </div>
                <input type="hidden" name="to" value="<?= $student_info['usr_id'] ?>">
